...
export const supabase = createClient(supabaseUrl, supabaseServiceRoleKey || 'fallback-key-to-prevent-crash');
...
export async function updateEmailStatus(emailLogId, status, errorMessage = null) {
  try {
    // ✅ FIXED: Use consistent UTC timestamp creation with error handling
    let createUTCTimestamp;
    try {
      ({ createUTCTimestamp } = await import('./timezone-utils.js'));
    } catch (importError) {
      console.warn('Failed to import timezone-utils, using fallback', { error: importError.message });
      createUTCTimestamp = () => new Date().toISOString();
    }
    
    const updateData = {
      status: status,
      delivered_at: status === 'delivered' ? createUTCTimestamp() : null,
...
export async function getCustomersWithOrders() {
  try {
    ...
    const { data: orders, error: orderError } = await supabase
      .from('orders')
      .select('customer_id, order_number, total_amount, final_amount, created_at, order_status')  // Correct column names
    ...
    const enhancedCustomers = customers.map(customer => {
      ...
      const totalAmount = customerOrders.reduce((sum, order) => {
          // Use final_amount (discounted) if available, otherwise total_amount
          const amount = parseFloat(order.final_amount) !== undefined && parseFloat(order.final_amount) !== null 
      ...
      const finalAmount = customer.payment_status === 'paid' && latestOrder
        ? (parseFloat(latestOrder.final_amount) !== undefined && parseFloat(latestOrder.final_amount) !== null
            ? parseFloat(latestOrder.final_amount)
            : parseFloat(latestOrder.total_amount) || 197.00)
        : 0;
